...
int ListSearch(List* l, const void* p) {
  int keep = l->currentPos; 
  struct _ListNode* currentSave = l->current;


  if(l->compare(l->head->item,p) > 0)return -1;
  else if(l->compare(p,l->tail->item) > 0)return -1;
  ...
  if(comp > 0){
    while(comp > 0){
      ...
      comp = l->compare(l->current->item,p);
      if(comp == 0)return 0;
    }
  }
  else if(comp < 0){
  ...
  l->currentPos = keep;
  l->current->item = currentSave->item;
  l->current->next = currentSave->next;
